...
def print_progress_bar(name, downloaded, total_size, length=20):
    """Print a progress bar to the console for download tracking."""
    # Set total_size to downloaded if it's zero or less to avoid division by zero
    if total_size <= 0:
        total_size = downloaded if downloaded > 0 else 1  # Ensure total_size is at least 1
    
    # Calculate percentage and filled length of the progress bar
    percent = "{0:.1f}".format(100 * (downloaded / float(total_size)))
    filled_length = int(length * downloaded // total_size)
    bar = '#' * filled_length + '-' * (length - filled_length)
    
    # Print the progress bar with the percentage, downloaded, and total size
    sys.stdout.write(f'\r{name} - [{bar}] {percent}% ({downloaded}/{total_size} bytes)')
    sys.stdout.flush()
    
    # Print "Completed" when download reaches 100%
    if downloaded >= total_size:
        sys.stdout.write(f'\r{name} - [{bar}] 100% (Completed)\n')
        sys.stdout.flush()
