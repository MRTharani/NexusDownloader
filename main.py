            ...
            if os.path.isfile(file_path):
                ...
                try:
                    ...
                    if file_size <= threshold_size:  # Check the condition properly
                        vid = await app.send_video(DUMP_ID, file_path, thumb=thumbnail_name, caption=vid_name)
        ...
        if video_links:
            for url in video_links[:10]:
              if url not in downloaded:
                ...
                response = add_links(device, url, "Videos")
                check_for_new_links(device, device.linkgrabber)
                # Usage examples:   
                draft_condition = lambda link: link['name'].endswith(('mp4', 'mkv'))
                jav_condition = lambda link: "rapidgator" not in link["url"]
                ph_condition = lambda link: "720" in link['name'] and "hls" in link['name']
                # Call the refactored function
                if "draftsex" in url :
                    move_links(device, draft_condition)
                elif "missav" in url:
                    move_links(device, jav_condition)
                elif "pornhub" in url:
                    move_links(device, ph_condition)
